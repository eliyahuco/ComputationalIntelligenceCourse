...
# Evaluate the classifiers for each number of users
def evaluate_by_user_count(clf, user_indices):
    X_train_subset = X_train_scaled[user_indices]
    y_train_subset = y_train[user_indices]
    return evaluate_classifier(clf, X_train_subset, y_train_subset, X_test_scaled, y_test)
...
def main():
    ...
    accs = []
    for i in range(1, 33):
        user_indices = [j for j, user in enumerate(users_train) if user <= i]
        acc = Parallel(n_jobs=-1, backend='threading')(
            delayed(evaluate_by_user_count)(clf, user_indices) for clf in classifiers
        )
        accs.append(acc)
